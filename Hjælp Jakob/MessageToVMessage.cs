...
{
	internal class MessageToVMessage : IMessage
	{
		...
		public MessageToVMessage(string to, string from, string body, string subject, string cc)
		{
			this.to = to;
			this.from = from;
			this.body = body;
		...
		{
			if (type.Equals(MessageCarrier.VMessage))  // man ville sagtens kunne fjerne denne if statement, men på et eller andet led synes jeg at det stadig giver mening at tjekke for om man har har valgt at sende den rigtige besked. igen, man kunne sagtens slette den, og det burde ikke gøre andet end at gøre det lettere for ens bruger. 
			{
				if (isHTML)
